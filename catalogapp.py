...
app = Flask(__name__)
# app.config['JSON_SORT_KEYS'] = False #uncomment it if you want to see
# unsorted JSON endpoint data.
...
def getLocalUser(email):
    # user = session.query(User).filter_by(email=email).one() commented out as
    # no need for password to be part of the object
    user = session.query(User).filter_by(
        email=email).with_entities(
        User.id,
        User.name,
        User.email,
        User.picture).one()
...
def itemExistCheck(title, name):
    ...
    try:
        session.query(Items).filter(
            Items.category_id == Categories.id).filter(
            Items.title == title).filter(
            Categories.name == name).one()
        check = 1
    except exc.SQLAlchemyError:
        pass
    return check
...
def itemOwnerCheck(title, name, user_id):
    ...
    try:
        session.query(Items).filter(
            Items.category_id == Categories.id).filter(
            Items.title == title).filter(
            Categories.name == name).filter(
                Items.user_id == user_id).one()
        check = 1
    except exc.SQLAlchemyError:
        pass
    return check
...
def showLogin():
    if request.method == 'POST':
        ...
        # create randome numbers of 32chars for state of the login session
        state = ''.join(
            random.choice(
...
def localUserLogin(email, password):
    if(authenticateLocalUser(email, password)):
        ...
        # items=items, currentUserName=currentUserName,showLatest="true")
...
def gconnect():
    ...
    # this might not work if you dont have the g+ account so changed from
    # 'name' to 'email'
...
def showCategoriesLatestItems():
    ...
    items = sorted(items, key=lambda items: items.title)
...
def showItemDescription(name, title):
    # now we have to seclect the description where which matches title from
    # item and name from catagories
    categories = getCategories()
...
def createNewItem():
    ...
    if request.method == 'POST':
        ...
        if(newItem):
            ...
            currentUserName = getLocalUser(login_session['email'])
            flash(
                'New Item %s already exist (may be create by someone else) ' %
                request.form['title'])
            return render_template(
                'catalog.html',
                categories=categories,
...
@app.route(
    '/catalog/<string:name>/<string:title>/edit',
    methods=[
        'GET',
        'POST'])
def editExistingItem(name, title):
    ...
    if request.method == 'POST':
        ...
        if(itemExistCheck(title, name)):
            ...
            flash(
                'Item %s does not exist that you want to edit' %
                request.form['title'])
            return redirect(url_for('showCategoriesLatestItems'))
...
@app.route(
    '/catalog/<string:name>/<string:title>/delete',
    methods=[
        'GET',
        'POST'])
...
def categoryJSON(category_id):
    if(session.query(Categories).filter_by(id=category_id).all()):
        item = session.query(Categories).filter_by(id=category_id).all()
        return jsonify(Category=[i.serialize for i in item])
    else:
        return jsonify(Category="Incorrect Category_id")
...
def itemJSON(category_id, item_id):
    if(session.query(Items)
            .filter(Items.category_id == category_id)
            .filter(Items.id == item_id)
            .all()):

        item = session.query(Items).filter(
            Items.category_id == category_id).filter(
            Items.id == item_id).all()

        return jsonify(Item=[i.serialize for i in item])
    else:
        return jsonify(item="Incorrect Category_id or item_id")
